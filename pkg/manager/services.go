	...
	if svc.Annotations[kubevip.Egress] == "true" && len(serviceIPs) > 0 {
		...
		if svc.Annotations[kubevip.EgressIPv6] == "true" {
			...
			if svc.Annotations[kubevip.ActiveEndpointIPv6] != "" {
				for _, serviceIP := range serviceIPs {
					if sm.config.EnableEndpointSlices && vip.IsIPv6(serviceIP) {
						...
						err = sm.configureEgress(serviceIP, podIP, svc.Namespace, string(svc.UID), svc.Annotations)
						if err != nil {
			...
			for _, serviceIP := range serviceIPs {
				...
				err = sm.configureEgress(serviceIP, podIPs, svc.Namespace, string(svc.UID), svc.Annotations)
				if err != nil {
					errList = append(errList, err)
	...
	if !shared {
		...
		if serviceInstance.ServiceSnapshot.Annotations[kubevip.Egress] == "true" {
			if serviceInstance.ServiceSnapshot.Annotations[kubevip.ActiveEndpoint] != "" {
				...
				err := services.TeardownEgress(serviceInstance.ServiceSnapshot.Annotations[kubevip.ActiveEndpoint], serviceInstance.ServiceSnapshot.Spec.LoadBalancerIP, serviceInstance.ServiceSnapshot.Namespace, string(serviceInstance.ServiceSnapshot.UID), serviceInstance.ServiceSnapshot.Annotations, sm.config.EgressWithNftables)
				if err != nil {
