...
func (t *Table) StartControlPlane(ctx context.Context, clusterManager *cluster.Manager) {
	if err := t.cpCluster.StartVipService(ctx, t.config, clusterManager, nil); err != nil {
...
func (t *Table) StartServices(ctx context.Context, id string) error {
	...
	if t.config.EnableServicesElection {
		if err := t.ServicesPerServiceLeader(ctx); err != nil {
			return err
		}
	} else if t.config.EnableLeaderElection {
		t.ServicesGlobalLeader(ctx, id)
	} else {
