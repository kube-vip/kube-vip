...
type BGPRouteManager interface {
	AddHost(ctx context.Context, addr string) error
	DelHost(ctx context.Context, addr string) error
}
	...
	for {
		...
		if healthy {
			...
			if !routeAnnounced {
				...
				if err := bgpServer.AddHost(ctx, vipCIDR); err != nil {
			...
			if consecutiveFailures >= c.ControlPlaneHealthCheck.FailureThreshold && routeAnnounced {
				...
				if err := bgpServer.DelHost(ctx, vipCIDR); err != nil {
