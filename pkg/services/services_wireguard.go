...
// updateServiceWireguardEndpoints updates DNAT rules when endpoints change
func (p *Processor) updateServiceWireguardEndpoints(ctx context.Context, svc *v1.Service) error {
	if !p.config.EnableWireguard {
		return nil
	}

	log.Debug("[wireguard] updating endpoints for service",
		"namespace", svc.Namespace,
		"name", svc.Name)
	...
	// Update DNAT rules for each VIP
	for _, vip := range serviceIPs {
		tunnelConfig := p.TunnelMgr.GetConfigForVIP(vip)
		if tunnelConfig == nil {
			log.Warn("[wireguard] no tunnel config found for VIP during endpoint update",
				"vip", vip,
				"service", svc.Name)
			continue
		}
		...
		for _, port := range svc.Spec.Ports {
			...
			log.Debug("[wireguard] updating DNAT rule for endpoint change",
				"service", svc.Name,
				"vip", vipAddr,
				"port", port.Port,
				"newTarget", targetEndpoint,
				"newTargetPort", actualTargetPort)

			// Delete old rule
			if err := nftables.DeleteIngressChains(isIPv6, portServiceID); err != nil {
				log.Warn("[wireguard] failed to delete old DNAT chain during update",
					"service", svc.Name,
					"port", port.Port,
					"err", err)
			}
			...
			if err != nil {
				log.Error("[wireguard] failed to apply updated DNAT rule",
					"service", svc.Name,
					"vip", vipAddr,
					"port", port.Port,
					"err", err)
			...
			log.Info("[wireguard] updated DNAT rule for endpoint change",
				"service", svc.Name,
				"vip", vipAddr,
				"port", port.Port,
				"target", fmt.Sprintf("%s:%d", targetEndpoint, actualTargetPort))
