...
func (s *controlPlaneFaultSuite) requireFaultMetricCapabilities() {
	s.metrics = hasMetricCapabilityWithLabels(
		s.ctx,
		s.cluster.Name,
		s.nodes,
	...
	if Mode == ModeARP && !hasMetricCapabilityOnAnyNode(s.ctx, s.cluster.Name, s.nodes, faultVIPAddressesMetric) {
		s.metrics = false
	}
	if !hasMetricCapabilityOnAnyNode(s.ctx, s.cluster.Name, s.nodes, s.operationMetric()) {
		s.metrics = false
	}
}
...
func (s *controlPlaneFaultSuite) assertLoopLiveness() {
	...
	if !hasMetricCapabilityWithLabels(s.ctx, s.cluster.Name, s.nodes, faultElectionLoopsMetric, electionLabels) {
		return
	}
	for _, node := range s.nodes {
	...
	if !hasMetricCapabilityOnAnyNode(s.ctx, s.cluster.Name, s.nodes, faultWatcherLoopsMetric) {
		By(withTimestamp("skipping node watcher loop assertion: this control-plane topology has no node watcher series"))
		return
	}
	if !hasMetricCapabilityWithLabels(s.ctx, s.cluster.Name, s.nodes, faultWatcherLoopsMetric, nodeWatcherLabels) {
		return
	}
	for _, node := range s.nodes {
		assertEventuallyStableMetric(
...
func (s *controlPlaneFaultSuite) assertVIPAddresses(leader string) {
	if !hasMetricCapabilityOnAnyNode(s.ctx, s.cluster.Name, s.nodes, faultVIPAddressesMetric) {
		return
	}
...
func (s *controlPlaneFaultSuite) assertQuietOperationCounter(fault string) {
	...
	if !hasMetricCapabilityOnAnyNode(s.ctx, s.cluster.Name, s.nodes, primary) {
		return
	}
	...
	for _, name := range []string{faultRouteOperationsMetric, faultBGPOperationsMetric, faultEgressOperationsMetric} {
		if name == primary || !hasMetricCapabilityOnAnyNode(s.ctx, s.cluster.Name, s.nodes, name) {
			continue
		}
...
func (s *controlPlaneFaultSuite) assertWatcherRestartDelta(before faultMetricSnapshot, shouldIncrement bool, fault string) {
	if !hasMetricCapabilityOnAnyNode(s.ctx, s.cluster.Name, s.nodes, faultWatcherRestartMetric) {
		return
	}
	...
	Eventually(func() (float64, error) {
		...
		time.Sleep(faultMetricGap)
		second := s.transitionSnapshot()
		if metricSnapshotTotal(first) != metricSnapshotTotal(second) {
			return 0, fmt.Errorf("watcher restart counter is still changing")
		}

		delta := metricSnapshotTotal(second) - metricSnapshotTotal(before)
		if delta < 0 {
...
func metricSnapshotTotal(snapshot faultMetricSnapshot) float64 {
	var total float64
	for _, metrics := range snapshot {
		total += e2e.SumMetric(metrics, faultWatcherRestartMetric, nil)
	}
	return total
}
